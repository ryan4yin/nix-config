...
| Concise commit            | A small, self-explanatory change is committed.                                                            | Use a short subject well under 50 characters; skip the body or keep it to context the diff does not show.             |
| Chained command           | The runtime tool description suggests chaining commands with `&&`.                                        | Follow the global rule: run separate commands or use Nushell/Python for the glue.                                     |

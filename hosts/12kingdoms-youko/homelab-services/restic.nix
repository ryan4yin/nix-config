...
in
{
  # DISABLED: this backup has never worked. It needs three agenix secrets that
  # do not exist in nix-secrets yet (`restic-password`, `ssh-key-for-restic-backup`,
  # and an rclone config with the `smb-downloads` remote; the existing
  # `rclone.conf` secret is only defined for the desktop secret set). Re-enable
  # once the secrets are provisioned.
  #
        ...
        backupPrepareCommand = ''
          ${pkgs.nushell}/bin/nu -c '
            ...
            kubevirt_nodes | each {|it|
              ^${pkgs.rsync}/bin/rsync -avz -e $"ssh -i ${sshKeyPath}" $"($it):/persistent/" $"/tmp/restic-backup-temp/($it)"
            }
          '
        '';

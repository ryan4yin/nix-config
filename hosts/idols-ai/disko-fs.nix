...
#   nixos-install --flake .#ai --option disko.devices.disk.main.device /dev/nvme1n1
{
  ...
  disko.devices = {
    ...
    disk.main = {
      type = "disk";
      # Override at install time if needed: --option disko.devices.disk.main.device /dev/nvme1n1
      device = "/dev/nvme1n1";
      content = {
        type = "gpt";
        partitions = {
          ...
          root = {
            ...
            content = {
              type = "luks";
              name = "crypted-nixos"; # Mapper name; must match boot.initrd.luks (disko adds it)
              settings = {
